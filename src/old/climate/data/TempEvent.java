...
public class TempEvent {
    ...
    public static TempEvent getTempEvent(long startTime, boolean isCold) {
        ...
        if (isCold) {
            ...
            peakTemp = WorldClimate.COLD_PERIOD_PEAK - (float) Math.abs(random.nextGaussian());
            bottomTemp = (float) (random.nextGaussian());
            switch (random.nextInt(10)) {
                case 0:
                    bottomTemp += WorldClimate.COLD_PERIOD_BOTTOM_T4;
                    break;
                case 1:
                case 2:
                    bottomTemp += WorldClimate.COLD_PERIOD_BOTTOM_T3;
                    break;
                case 3:
                case 4:
                case 5:
                    bottomTemp += WorldClimate.COLD_PERIOD_BOTTOM_T2;
                    break;
                default:
                    bottomTemp += WorldClimate.COLD_PERIOD_BOTTOM_T1;
                    break;
            }
        }
        ...
        // Warm Period
            ...
            peakTemp = WorldClimate.WARM_PERIOD_PEAK - (float) Math.abs(random.nextGaussian());
        }
    ...
    public float getHourTemp(long t) {
        ...
        if (isCold) {
            if (t >= startTime && t < peakTime) {
                return getPiecewiseTemp(t, startTime, peakTime, WorldClimate.CALM_PERIOD_BASELINE, peakTemp, 0, 0);
                ...
                return getPiecewiseTemp(t, bottomTime, endTime, bottomTemp, WorldClimate.CALM_PERIOD_BASELINE, 0, 0);
            } else if (t >= endTime && t <= calmEndTime) {
                return WorldClimate.CALM_PERIOD_BASELINE + (float) (random.nextGaussian());
            } else {
                return WorldClimate.CALM_PERIOD_BASELINE + (float) (random.nextGaussian());
            }
        }
        if (t >= startTime && t < peakTime) {
            ...
            return getPiecewiseTemp(t, peakTime, endTime, peakTemp, WorldClimate.CALM_PERIOD_BASELINE, 0, 0);
        } else if (t >= endTime && t <= calmEndTime) {
            return WorldClimate.CALM_PERIOD_BASELINE + (float) (random.nextGaussian());
        } else {
            return WorldClimate.CALM_PERIOD_BASELINE + (float) (random.nextGaussian());
