...
import com.pouffydev.create_freezedown.foundation.climate.CTPacketHandler;
...
public class CTTemperatureDisplayPacket {
    ...
    public void handle(Supplier<NetworkEvent.Context> context) {
        context.get().enqueueWork(() -> {
            ...
            Component tosend= CTLang.translate("message."+langKey,ss).component();
            if(isStatus)
    ...
    public static void send(ServerPlayer pe, String format, int...temps) {
        CTPacketHandler.getChannel().send(PacketDistributor.PLAYER.with(()->pe),new CTTemperatureDisplayPacket(format,temps));
    }
    ...
    public static void send(Collection<ServerPlayer> pe, String format, int...temps) {
        ...
        for(ServerPlayer p:pe)
            CTPacketHandler.getChannel().send(PacketDistributor.PLAYER.with(()->p),k);
    }
    ...
    public static void sendStatus(ServerPlayer pe,String format,boolean act,int...temps) {
        CTPacketHandler.getChannel().send(PacketDistributor.PLAYER.with(()->pe),new CTTemperatureDisplayPacket(format,act,temps));
    }
