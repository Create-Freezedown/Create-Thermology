...
public class BronzeBoilerData {
    ...
    public boolean boilerTankAbove;
    // display
    ...
    public void tick(BronzeTankBlockEntity controller) {
        if (!isActive())
            return;
        ...
        boilerTankAbove = controller.getLevel().getBlockEntity(controller.getBlockPos().above()) instanceof BoilerTankBlockEntity;
    ...
    public boolean addToGoggleTooltip(List<Component> tooltip, boolean isPlayerSneaking, int boilerSize) {
        ...
        int tankLevel = Math.min(activeHeat, Math.min(maxHeatForWater, maxHeatForSize));
        int boilerLevel = Math.min(maxHeatForWater, maxHeatForSize);
        double totalSU = getEngineEfficiency(boilerSize) * 16 * Math.max(tankLevel, boilerLevel)
                * BlockStressValues.getCapacity(AllBlocks.STEAM_ENGINE.get());
        
        tooltip.add(Components.immutableEmpty());
        
        if (boilerLevel > 0 && maxHeatForSize > 0 && maxHeatForWater == 0 && (passiveHeat ? 1 : activeHeat) > 0) {
        ...
        Lang.number(totalSU)
                ...
                .space()
                .add((Lang.translate("boiler.via_one_engine")).style(ChatFormatting.DARK_GRAY))
                .forGoggles(tooltip, 1);
    ...
    public boolean updateTemperature(BronzeTankBlockEntity controller) {
        ...
        for (int xOffset = 0; xOffset < controller.width; xOffset++) {
            for (int zOffset = 0; zOffset < controller.width; zOffset++) {
                ...
                BlockState blockState = level.getBlockState(pos);
                float heat = BronzeTankHeaters.getActiveHeat(level, pos, blockState);
                if (heat == 0) {
                    passiveHeat = true;
                } else if (heat > 0) {
                    activeHeat += heat;
    ...
    public boolean isActive() {
        return attachedWhistles > 0 || boilerTankAbove;
    }
    ...
    public CompoundTag write() {
        ...
        nbt.putBoolean("BoilerTankAbove", boilerTankAbove);
        return nbt;
    }
    ...
    public void read(CompoundTag nbt, int boilerSize) {
        ...
        boilerTankAbove = nbt.getBoolean("BoilerTankAbove");
